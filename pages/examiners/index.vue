...
export default {
  ...
  async asyncData({ $axios, $auth, $config }) {
    const events = await $axios.get(
      `api/event/getByExaminer?examiner=${$auth.$state.user.id}`
    );
    console.log(events);
    return { events };
  },
